    ...
    <div class="container-fluid  dashboard-content">
        ...
        <div class="row">
            <div class="card col-lg-8">
                {{--		<form method="post" action="{{url('/adminstrator/articles/delete')}}">--}}
                {{--			{{ csrf_field() }}--}}
                <div class="py-3 px-2">
                    ...
                    {{--					حذف انتخاب شده ها--}}
                    {{--				</button>--}}
                    <a href="{{url('adminstrator/pages/add')}}" type="button" class="btn btn-space btn-primary">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                ...
                <div class="row w-100 m-0">
                    ...
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        <div class="card">
                            {{--						<h5 class="card-header">صفحات ایستا</h5>--}}
                                                ...
                                                <table class="table table-striped table-bordered first dataTable"
                                                    ...
                                                    @foreach($articles as $key=> $article)
                                                        <tr role="row" class="odd">
                                                            ...
                                                            <td class="sorting_1">            {{($articles->appends(Request::except('page'))->currentPage() - 1) *
                                          ($articles->appends(Request::except('page'))->perPage()) + $key+1}}</td>
                                                            <td class="sorting_1">{{@$article->title}}</td>


                                                            <td class="sorting_1">
                                                                <img
                                                                    src="{{asset('assets/uploads/content/page/small/'.$article->image)}}"
                                                                    width="70" height="70"/>
                                                            </td>
                                                            <td class="sorting_1">
                                                                {{@$article['status'] == 1 ? 'نمایش در سایت' : 'عدم نمایش در سایت'}}
                                                                {{@$article['footer'] == 1 ? 'نمایش در فوتر' : 'عدم نمایش در فوتر'}}
                                                                {{@$article['menu'] == 1 ? 'نمایش در منو' : 'عدم نمایش در منو'}}
                                                            </td>
                                                            <td>
                                                                <a href="{{URL::action('Admin\IstaController@getEdit',$article->id)}}"
                                                                   type="button"
                                                                   class="btn btn-space btn-warning"
                                                                   data-toggle="tooltip" title="ویرایش">
                                                                    <i class="fa fa-edit"> </i>
                                                                </a>
                                                                <a href="{{URL::action('Admin\IstaController@getDelete',$article->id)}}"
                                                                   type="button"
                                                                ...
                                                                <a href="{{route('site.page.detail',['id'=>$article->url])}}"
            ...
            <div class="col-lg-4">
                <div id="list">
                    <div class="alert alert-info alert-dismissable rounded-lg"
                         style="direction: rtl; margin: 0px auto;">
                    ...
                    <ul>
                        <hr>
                        <div class="alert alert-primary alert-dismissable rounded-lg"
                             style="direction: rtl; margin: 0px auto;">
                            <i class="fa fa-arrow-circle-down"></i>
                            <span style="font-size: 10px;">	ترتیب ها  </span>
                        </div>
                        @foreach($categorySort as $rowSort)
                            <li class="list-unstyled  my-2 p-3 shadow-sm rounded-lg"
                                id="arrayorder_{!! stripslashes($rowSort['id']) !!}"
                                style="background-color: #333">{!! stripslashes($rowSort['title']) !!}
                                <div class="clear"></div>
                            </li>
            ...
            $(function () {
                $("#list ul").sortable({
                    opacity: 0.8, cursor: 'move', update: function () {
                        ...
                        $.post("{!!URL::action('Admin\IstaController@postSort')!!} ", order, function (theResponse) {
