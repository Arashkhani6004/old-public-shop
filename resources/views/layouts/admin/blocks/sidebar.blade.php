<div class="nav-left-sidebar sidebar-dark" style="height: 92% !important;">
	<div class="menu-list">
		<nav class="navbar navbar-expand-lg navbar-light">
			...
			<div class="collapse navbar-collapse" id="navbarNav">
				<ul class="navbar-nav flex-column">
                    ...
                    <a class="nav-link active" href="{{url('/')}}" target="_blank">
                        <i class="fa fa-fw fa-sitemap"></i>
                       صفحه اصلی سایت
                    </a>
                    <a class="nav-link" href="{{url('/adminstrator')}}">
                        <i class="fa fa-fw fa-user-circle"></i>
                        داشبورد
                    </a>
                    <a class="nav-link" href="{{url('adminstrator/shipment')}}">
                        <i class="fa fa-fw fa-user-circle"></i>
                        روش ارسال
                    </a>
					<li class="nav-item ">
...
{{--						</div>--}}
						...
						<div id="submenu-2" class="collapse submenu" style="">
							<ul class="nav flex-column">
								...
								@endif
                                    ...
                                    @endif
                                    @if(Auth::user()->hasPermission('sloagen'))
                                        <li class="nav-item">
                                            <a class="nav-link"
                                               href="{{URL::action('Admin\SloagenController@get')}}">شعار ها</a>
                                        </li>
                                    @endif
							</ul>
						</div>
							...
							<ul class="nav flex-column">
								...
								@endif
                                    @if(Auth::user()->hasPermission('mobile-slider'))
                                        <li class="nav-item">
                                            <a class="nav-link"
                                               href="{{URL::action('Admin\SliderController@getMobile')}}">اسلایدر موبایل</a>
                                        </li>
                                    @endif
                                    ...
                                    @endif
                                    @if(Auth::user()->hasPermission('users-questions'))
                                        <li class="nav-item">
                                            <a class="nav-link"
                                               href="{{URL::action('Admin\UsersQuestionController@getList')}}">
                                                سوالات متداول کاربران</a>
                                        </li>
                                    @endif
                                    @if(Auth::user()->hasPermission('tags'))
                                        <li class="nav-item">
                                            <a class="nav-link"
                                               href="{{URL::action('Admin\TagController@get')}}">تگ ها</a>
                                        </li>
                                    @endif
                                    @if(Auth::user()->hasPermission('pages'))
                                        <li class="nav-item">
                                            <a class="nav-link"
                                               href="{{URL::action('Admin\IstaController@get')}}">صفحات ایستا</a>
                                        </li>
                                    @endif
                    ...
                    @php
                        $comcount =App\Models\Comment::where('status','
                       <>','1')->count();
                        $tickcount =App\Models\Ticket::whereNull('parent_id')->whereStatus(0)->count();
                           $concount =App\Models\Contact::where('readat','0')->count();
                    @endphp
					<li class="nav-item">
						<a class="nav-link" href="#" data-toggle="collapse" aria-expanded="false"
							data-target="#submenu-5" aria-controls="submenu-5"><i
								class="fas fa-fw fa-inbox"></i>پیام ها

                            <span class="badge badge-danger rounded-circle ms-2">
									{{ $comcount +  $tickcount + $concount}}
								</span>
                        </a>
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="#" data-toggle="collapse" aria-expanded="false" data-target="#submenu-7" aria-controls="submenu-7"><i class="fas fa-fw fa-table"></i> فاکتورها </a>
                        <div id="submenu-7" class="collapse submenu" style="">
                            <ul class="nav flex-column">
                                @if(Auth::user()->hasPermission('order'))
                                @php $orderseen =App\Models\Order::where('seen','0')->count();
                                        @endphp
                                    <li class="nav-item">
                                        <a class="nav-link" href="{{URL::action('Admin\OrderController@getIndex')}}">همه فاکتورها
                                        <span class="badge badge-danger rounded-circle ms-2">
                                                {{@$orderseen}}
                                            </span>
                                        </a>
                    ...
                    <li class="nav-item">
                        ...
                        <div id="submenu-9" class="collapse submenu" style="">
                            <ul class="nav flex-column">
                                ...
                                @endif
                                @if(Auth::user()->hasPermission('sitemap'))
                                <li class="nav-item">
                                    <a class="nav-link" href="{{URL::action('Admin\SitemapController@getIndex')}}">سایت مپ</a>
                                </li>
                                @endif
                            </ul>
                        </div>
