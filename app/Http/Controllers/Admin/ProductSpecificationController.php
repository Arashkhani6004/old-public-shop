...
class ProductSpecificationController extends Controller
{
    public function getIndex($product_id)
    {
        ...
        $data = ProductSpecification::whereProductId($product_id)->with('productSpecificationType', 'productSpecificationValue')->orderby('id','DESC')->get();
    ...
    public function postAdd($product_id, Request $request)
    {
        ...
        $product = Product::find($product_id);
        $inputInsert = [];
        foreach ($input['spf_id'] as $item) {
            if(@$input['category_id'][intval($item)] !== null){
                $sp = ProductSpecification::create([
                    'product_specification_value_id' => $input['category_id'][intval($item)],
                    'product_specification_type_id' => $item,
...
//========================== PRODUCT IMAGE =================================================
    ...
    public function postAddImage(Request $request)
    {
        ...
        return \redirect('/adminstrator/sp/image/'.$input['product_specification_id'])->with('success','آیتم موردنظر با موفقیت ثبت شد.');
    }
    ...
    public function postEditImage($id , Request $request)
    {
        ...
        $log = Logs::log(url()->current(),$serialized_array,Auth::id(),$image->id);
        return \redirect('/adminstrator/sp/image/'.$image->product_specification_id)->with('success','آیتم موردنظر با موفقیت ویرایش شد.');
    }
