...
class DiscountController extends Controller
{
    ...
    public  function postAdd(Request $request) {
        $input = $request->all();
          if ($input['from_date'] == null){
            return redirect()->back()->with('error','لطفا  تاریخ شروع اعتبار  را وارد کنید ');
        }
        else{
  ...
  if ($input['to_date'] == null){
            return redirect()->back()->with('error','لطفا   تاریخ پایان اعتبار   را وارد کنید ');
        }
        else{
        ...
        $z =  Helper::persian2LatinDigit($input['amount']);
        $input['amount'] = $z;
        $discounts = [];
